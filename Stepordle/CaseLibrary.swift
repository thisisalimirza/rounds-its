...
class CaseLibrary {
    static func getSampleCases() -> [MedicalCase] {
        ...
        // Autogenerate additional cases from DiagnosisLexicon to reach at least 365
        let targetCount = 365
        if cases.count < targetCount {
            var existing = Set(cases.map { $0.diagnosis.lowercased() })
            for term in DiagnosisLexicon.all {
                let name = term.trimmingCharacters(in: .whitespacesAndNewlines)
                let lower = name.lowercased()
                guard !name.isEmpty, !existing.contains(lower) else { continue }
                let category = inferCategory(for: name)
                let hints = generateHints(for: name, category: category)
                ...
                if cases.count >= targetCount { break }
            }
        }
        return cases
    }
    ...
    // MARK: - Helpers for autogenerated cases
    private static func inferCategory(for diagnosis: String) -> String {
        let d = diagnosis.lowercased()
        // Cardiology
        if d.contains("mi") || d.contains("myocard") || d.contains("angina") || d.contains("fibrillation") || d.contains("tachycardia") || d.contains("endocard") || d.contains("pericard") || d.contains("valve") || d.contains("hypertension") || d.contains("aortic") || d.contains("cardio") {
            return "Cardiology"
        }
        // Neurology
        if d.contains("stroke") || d.contains("sah") || d.contains("subarachnoid") || d.contains("seizure") || d.contains("parkinson") || d.contains("alzheimer") || d.contains("als") || d.contains("multiple sclerosis") || d.contains("myasthenia") || d.contains("guillain") || d.contains("headache") || d.contains("neurop") {
            return "Neurology"
        }
        // Pulmonology
        if d.contains("asthma") || d.contains("copd") || d.contains("pneumonia") || d.contains("pulmonary") || d.contains("pneumothorax") || d.contains("sarcoid") || d.contains("ards") || d.contains("tb") || d.contains("tuberculosis") {
            return "Pulmonology"
        }
        // Gastroenterology
        if d.contains("gerd") || d.contains("reflux") || d.contains("ulcer") || d.contains("gastrit") || d.contains("crohn") || d.contains("colitis") || d.contains("hepat") || d.contains("cirrhos") || d.contains("pancreat") || d.contains("cholecyst") || d.contains("cholang") || d.contains("ibs") || d.contains("bowel") || d.contains("appendic") || d.contains("gi") {
            return "Gastroenterology"
        }
        // Endocrinology
        if d.contains("diabet") || d.contains("thyroid") || d.contains("cushing") || d.contains("addison") || d.contains("pheochrom") || d.contains("acromeg") || d.contains("parathy") || d.contains("pku") || d.contains("msud") || d.contains("insipidus") || d.contains("siadh") {
            return "Endocrinology"
        }
        // Nephrology
        if d.contains("kidney") || d.contains("nephro") || d.contains("glomerulo") || d.contains("uti") || d.contains("pyelo") || d.contains("nephrolith") || d.contains("rta") || d.contains("ckd") || d.contains("aki") {
            return "Nephrology"
        }
        // Hematology
        if d.contains("anemia") || d.contains("leukemia") || d.contains("lymphoma") || d.contains("myeloma") || d.contains("hemophilia") || d.contains("thrombocyt") || d.contains("sickle") || d.contains("thalassem") || d.contains("coagul") || d.contains("apl") {
            return "Hematology"
        }
        // Infectious Disease
        if d.contains("sepsis") || d.contains("meningit") || d.contains("encephalit") || d.contains("lyme") || d.contains("rocky") || d.contains("malaria") || d.contains("hiv") || d.contains("syphilis") || d.contains("gonorr") || d.contains("chlamydia") || d.contains("c. diff") || d.contains("clostridioides") || d.contains("botul") || d.contains("tetanus") || d.contains("influenza") || d.contains("covid") {
            return "Infectious Disease"
        }
        // Rheumatology
        if d.contains("lupus") || d.contains("rheumatoid") || d.contains("scleroderma") || d.contains("polymyositis") || d.contains("dermatomyositis") || d.contains("gout") || d.contains("pseudogout") || d.contains("ankylosing") || d.contains("vasculitis") || d.contains("arteritis") || d.contains("takayasu") || d.contains("pan") {
            return "Rheumatology"
        }
        // Psychiatry
        if d.contains("depress") || d.contains("bipolar") || d.contains("schizoph") || d.contains("anxiety") || d.contains("ocd") || d.contains("ptsd") || d.contains("adhd") || d.contains("autism") || d.contains("eating") {
            return "Psychiatry"
        }
        // Dermatology
        if d.contains("psoriasis") || d.contains("dermatitis") || d.contains("urticaria") || d.contains("pemphig") || d.contains("cellulitis") || d.contains("impetigo") || d.contains("tinea") || d.contains("melanoma") || d.contains("basal cell") || d.contains("squamous cell") {
            return "Dermatology"
        }
        // OB/GYN
        if d.contains("pregnan") || d.contains("preeclamp") || d.contains("eclamp") || d.contains("placenta") || d.contains("gestational") || d.contains("polycystic ovary") || d.contains("pcos") || d.contains("endometriosis") || d.contains("pid") || d.contains("fibroid") || d.contains("choriocarc") {
            return "Obstetrics & Gynecology"
        }
        // Pediatrics
        if d.contains("kawasaki") || d.contains("measles") || d.contains("mumps") || d.contains("rubella") || d.contains("varicella") || d.contains("bronchiolitis") || d.contains("croup") || d.contains("tay-sachs") || d.contains("congenital") || d.contains("down syndrome") || d.contains("turner") || d.contains("scfe") {
            return "Pediatrics"
        }
        // Urology
        if d.contains("prostat") || d.contains("testicular") || d.contains("varicocele") || d.contains("hydrocele") || d.contains("bph") {
            return "Urology"
        }
        // Ophthalmology / ENT
        if d.contains("glaucoma") || d.contains("cataract") || d.contains("macular") || d.contains("otitis") || d.contains("sinusitis") {
            return "Ophthalmology / ENT"
        }
        // Surgery / Emergency
        if d.contains("appendicitis") || d.contains("cholecystitis") || d.contains("diverticulitis") || d.contains("obstruction") || d.contains("hernia") || d.contains("aaa") || d.contains("abdominal aortic aneurysm") || d.contains("compartment") || d.contains("necrotizing") || d.contains("traumatic") {
            return "Surgery / Emergency"
        }
        // Fallback
        return "General Medicine"
    }

    private static func generateHints(for diagnosis: String, category: String) -> [String] {
        return [
            "Clinical scenario suggests \(diagnosis).",
            "Category: \(category). Consider typical Step 1 symptoms and risk factors.",
            "Workup: labs and imaging commonly used for \(diagnosis).",
            "Key distinguishing features often include hallmark findings for \(diagnosis).",
            "Management: start with first-line therapies; review contraindications and complications."
        ]
    }

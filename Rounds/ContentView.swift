...
struct ContentView: View {
    ...
    private func startNewGame() {
        // The scheduled case wins when the day has one.
        //
        // The seeded fallback below picks by *index* into the library array,
        // so its answer depends on the array's length and order: adding a
        // single case silently changes the daily case for every date, past and
        // future. That was survivable while the library only changed at
        // release. Now that cases are edited server-side it is not, so the
        // schedule is explicit — and curatable.
        //
        // Looked up on the device's own calendar day, matching the seed below
        // and the way every daily puzzle behaves: a new case at your local
        // midnight, not the server's.
        if let scheduled = CaseStore.shared.scheduledCase(for: Date()) {
            currentCase = scheduled
            isDailyCase = true
            showingGame = true
            return
        }

        // Fallback for any day the schedule doesn't cover, and the only path
        // on builds that predate it. Kept so the two agree during rollout: the
        // schedule was generated by reproducing exactly this calculation.
    ...
    ///
    /// Note this reads the *device's* calendar, so despite the long-standing
    /// comment claiming otherwise, this has never been the same for all users
    /// worldwide at the same moment — it is the same for everyone in a given
    /// timezone, rolling over at each user's local midnight. That is the usual
    /// daily-puzzle convention and the schedule preserves it deliberately.
